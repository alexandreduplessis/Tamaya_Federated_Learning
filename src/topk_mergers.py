...
class Merger_FedTopKHess:
    ...
    def __call__(self, outputs, accs_list):
        ...
        if self.loss_history is None:
            ...
            losses_except =  {output.client_id: np.absolute(output.losses[-1] - self.loss_history[output.client_id]) for output in outputs}
            self.loss_history = {output.client_id: output.losses[-1] for output in outputs}
            if k < 0: top_k = heapq.nsmallest(nk, losses_except, key=lambda cid: losses_except[cid])
            else: top_k = heapq.nlargest(nk, losses_except, key=lambda cid: losses_except[cid])
...
class Merger_FedTopKnegHess:
    ...
    def __call__(self, outputs, accs_list):
        ...
        # keep only clients with accuracy < 0.8
            ...
            losses_except =  {output.client_id: -(output.losses[-1] - self.loss_history[output.client_id]) for output in outputs}
            self.loss_history = {output.client_id: output.losses[-1] for output in outputs}
            if k < 0: top_k = heapq.nsmallest(nk, losses_except, key=lambda cid: losses_except[cid])
            else: top_k = heapq.nlargest(nk, losses_except, key=lambda cid: losses_except[cid])
